...
/*
CUDA kernel code for above
__global__ void conv_forward_kernel(double* input_tensor, double* kernel_weights, double* kernel_bias_weights, double* output_tensor, int input_tensor_channels, int output_tensor_channels, int input_side_size, int kernel_size, int output_side_size, int stride)
{
    // Calculate global indexes for output tensor
    int x_slide = blockIdx.x * blockDim.x + threadIdx.x;
    int y_slide = blockIdx.y * blockDim.y + threadIdx.y;
    int out_ch_cnt = blockIdx.z * blockDim.z + threadIdx.z;

    if (x_slide < output_side_size && y_slide < output_side_size && out_ch_cnt < output_tensor_channels)
    {
        // Make the dot product of input tensor with kernel weight for one kernel position
        double dot_product = kernel_bias_weights[out_ch_cnt]; // start with bias value for the dot product
        
        __shared__ double kernel_weights_shared[INPUT_TENSOR_CHANNELS][KERNEL_SIZE][KERNEL_SIZE];
        
        // Copy kernel weights for this channel to shared memory
        for (int in_ch_cnt = 0; in_ch_cnt < input_tensor_channels; in_ch_cnt++) {
            for (int ky = threadIdx.y; ky < kernel_size; ky += blockDim.y) {
                for (int kx = threadIdx.x; kx < kernel_size; kx += blockDim.x) {
                    kernel_weights_shared[in_ch_cnt][ky][kx] = kernel_weights[out_ch_cnt * input_tensor_channels * kernel_size * kernel_size + in_ch_cnt * kernel_size * kernel_size + ky * kernel_size + kx];
                }
            }
        }
        ...
        // Calculate input tensor position for this kernel position
        int inp_tens_y_pos = y_slide * stride;
        int inp_tens_x_pos = x_slide * stride;
        ...
        for (int ky = 0; ky < kernel_size; ky++)
        {
            for (int kx = 0; kx < kernel_size; kx++)
            {
                for (int in_ch_cnt = 0; in_ch_cnt < input_tensor_channels; in_ch_cnt++)
                {
                    dot_product += input_tensor[in_ch_cnt * input_side_size * input_side_size + (inp_tens_y_pos + ky) * input_side_size + (inp_tens_x_pos + kx)] * kernel_weights_shared[in_ch_cnt][ky][kx];
                }
            }
        }
        ...
        // Make the activation function
        // Put the dot product to output tensor map
        output_tensor[out_ch_cnt * output_side_size * output_side_size + y_slide * output_side_size + x_slide] = activation_function(dot_product);
    }
}

OR 

__global__ void conv_forward_kernel(double *input_tensor, double *kernel_weights, double *kernel_bias_weights, double *output_tensor, int input_tensor_channels, int output_tensor_channels, int input_side_size, int output_side_size, int kernel_size, int stride)
{
    int out_ch_cnt = blockIdx.x * blockDim.x + threadIdx.x;
    int y_slide = blockIdx.y * blockDim.y + threadIdx.y;
    int x_slide = blockIdx.z * blockDim.z + threadIdx.z;
    if (out_ch_cnt < output_tensor_channels && y_slide < output_side_size && x_slide < output_side_size) {
        double dot_product = kernel_bias_weights[out_ch_cnt];
        for (int ky = 0; ky < kernel_size; ky++)
        {
            int inp_tens_y_pos = ky + y_slide * stride;
            for (int kx = 0; kx < kernel_size; kx++)
            {
                ...
                for (int in_ch_cnt = 0; in_ch_cnt < input_tensor_channels; in_ch_cnt++)
                {
                    dot_product += input_tensor[in_ch_cnt * input_side_size * input_side_size + inp_tens_y_pos * input_side_size + inp_tens_x_pos] * kernel_weights[out_ch_cnt * input_tensor_channels * kernel_size * kernel_size + in_ch_cnt * kernel_size * kernel_size + ky * kernel_size + kx];
                }
            }
        }
        output_tensor[out_ch_cnt * output_side_size * output_side_size + y_slide * output_side_size + x_slide] = dot_product;
    }
}

This kernel function takes in the input tensor, kernel weights, kernel bias weights, output tensor, and other necessary parameters for convolution. The kernel code is executed by many threads in parallel, with each thread responsible for computing a single output element of the output tensor.
To use this kernel, you would need to allocate memory on the GPU for the input tensor, kernel weights, kernel bias weights, and output tensor, and copy the data from the CPU to the GPU. Then, you can launch the kernel using the <<<>>> syntax:

dim3 blockDim(8, 8, 8);
dim3 gridDim((output_tensor_channels + blockDim.x - 1) / blockDim.x, (output_side_size + blockDim.y - 1) / blockDim.y, (output_side_size + blockDim.z - 1) / blockDim.z);
conv_forward_kernel<<<gridDim, blockDim>>>(d_input_tensor, d_kernel_weights, d_kernel_bias_weights, d_output_tensor, input_tensor_channels, output_tensor_channels, input_side_size, output_side_size, kernel_size, stride);

In this example, we're launching the kernel with a block size of (8, 8, 8) and a grid size of (output_tensor_channels / 8 + 1, output_side_size / 8 + 1, output_side_size / 8 + 1). This means that each block will have 8 x 8 x 8 = 512 threads, and the total number of threads launched will be (output_tensor_channels / 8 + 1) * (output_side_size / 8 + 1) * (output_side_size / 8 + 1).
Inside the kernel, we're using the blockIdx and threadIdx variables to compute the current output element that each thread is responsible for. We're using the input_tensor_channels, input_side_size, output_side_size, kernel_size, and stride parameters to compute the input tensor and kernel

Yes, both examples should calculate the same output, but the second example should be much faster due to the use of parallelism in the CUDA kernel. However, it's important to note that the parallel version may produce slightly different results due to floating-point rounding errors that can arise when calculations are performed in a different order. This is generally not a significant issue for most applications, but it's something to be aware of when comparing results from different implementations.
*/
