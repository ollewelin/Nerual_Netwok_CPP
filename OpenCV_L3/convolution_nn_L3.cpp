...
int main()
{
   ...
   // ======== create 3 convolution layer objects =========
    ...
    cv::Mat Mat_L3_output_visualize(one_plane_L3_out_conv_size, conv_L3.output_tensor.size() * space_grid + conv_L3.output_tensor.size() * one_plane_L3_out_conv_size , CV_32F);//Show a full pattern of L3 output convolution signals one rectangle for each output channel of L3 conv
                    ...
                    for (int ic = 0; ic < input_channels; ic++)
                    {
                        for (int yi = 0; yi < one_side; yi++)
                        {
                            for (int xi = 0; xi < one_side; xi++)
                            {
                                ...
                                upsamp_visualization_single_kernels_Mat.at<float>(ic * one_side + yi, xi + L3_oc*space_grid + L3_oc*one_side) = (float)input_pixel_data;
                            }
                        }
                    }
                }            
