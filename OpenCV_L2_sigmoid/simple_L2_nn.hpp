#ifndef SIMPLE_L2_NN_H
#define SIMPLE_L2_NN_H
#include<vector>
#include<string>

using namespace std;
class simple_L2_nn
{
private:
    string filename_hid_L1;
    string filename_hid_L2;
    string filename_out;
    int input_nodes;
    int hidden_L1_nodes;
    int hidden_L2_nodes;
    int output_nodes;
    vector<double> hidden_L1_layer;
    vector<double> hidden_L1_delta;
    vector<double> hidden_L2_layer;
    vector<double> hidden_L2_delta;
    vector<double> output_delta;
    vector<vector<double>> hid_L1_weights;//hid_L1_weights[to_node][from_node]
    vector<vector<double>> hid_L1_change_weights;//[to_node][from_node]
    vector<vector<double>> hid_L2_weights;//hid_L1_weights[to_node][from_node]
    vector<vector<double>> hid_L2_change_weights;//[to_node][from_node]
    vector<vector<double>> out_weights;//[to_node][from_node]
    vector<vector<double>> out_change_weights;//[to_node][from_node]

public:
    double loss;
    double learning_rate;
    double momentum;
    int use_softmax;
    vector<double> input_layer;
    vector<double> output_layer;
    vector<double> target_layer;


    void randomize_weights(double);//the input argument must be in range 0..1 but to somthing low for example 0.01
    void load_weights(void);//load weights with file name argument 
    void save_weights(void);//save weights with file name argument 
    void forward_pass(void);
   // void only_loss_calculation(void);
    void backpropagtion_and_update(void);//If batchmode update only when batch end

    
    simple_L2_nn();
    ~simple_L2_nn();
};




#endif//SIMPLE_L2_NN_H
