...
private:
    ...
    vector<vector<vector<double>>> delta_gamma;//3D [channels][row][col]. delta for gamma
    vector<vector<vector<double>>> delta_beta;//3D [channels][row][col]. delta for beta
...
public:
    ...
    int backprop_batch(void);//return sample coundter
